...
def _segment_confidence(source_level: str, used_n: int, condition: str, new_n: int) -> str:
    if condition == "new":
        if source_level in {"brand+ram+family", "brand+ram", "brand+family"} and new_n >= 3:
            return "medium"
        if source_level in {"brand", "ram"} and new_n >= 3:
            return "low"
        return "very_low"
...
def _compute_deal_rating_new(
    price_asked: Optional[float],
    fair_price_new: Optional[float],
) -> Dict[str, Any]:
    if price_asked is None:
        return {
            "label": "unknown",
    ...
    rel = abs(price_asked - fair_price_new) / max(fair_price_new, 1.0)

    if price_asked < fair_price_new * 0.90:
        return {
            "label": "very_good",
            "score": 90,
            "delta_vs_fair": delta,
    ...
    if rel <= 0.10:
        score = int(round(85 - 30 * rel / 0.10))
        score = int(_clamp(score, 70, 85))
        return {
            "label": "fair",
            "score": score,
...
def estimate_price(
    ...
    if condition == "new":
        fair_price_selected = fair_price_new
        deal = _compute_deal_rating_new(
        ...
        deal = _compute_deal_rating_used(
            price_asked=price_asked,
            q1=q1,
            q3=q3,
